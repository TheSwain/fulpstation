/datum/job/fulp/brigdoc //[FULP] [XEON] [PNX] Brig Doctor job, a modified version Xeon's EMT job.
	...
	exp_requirements = 180 // Requires 5 or so hours of playtime on medical and security. I think.
...
/datum/outfit/job/brigdoc //Brig Doc outfit
	...
	uniform = /obj/item/clothing/under/rank/medical/doctor // Just the standard issue Medical Doctor jumpsuit
	...
	pda_slot = SLOT_L_STORE // Belt slot is already occupied so the PDA has to go into a pocket. On a side note, thank god this exists; the old method we used was spawning in an unused PDA which sucked.
